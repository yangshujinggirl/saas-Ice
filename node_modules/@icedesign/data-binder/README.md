...
本视频的 Demo 代码在：<http://gitlab.alibaba-inc.com/ice-assets/ice-data-binder-demo/branches> 下面，其中 `use-ajax` 分支是 AJAX 的方式书写，`use-data-binder` 分支上代码是使用 Data-Binder 方案实现的同等效果代码。
...
class ListView extends Component {
  ...
  render() {
    ...
    return (
      <div>
        <Loading state={account.__loading ? 'on' : 'off'} shape="fusion-reactor">
...
```json
{
  ...
  "data": {
    "foo": "data 是具体的数据，需要与该接口定义的 defaultBindingData 字段结构保持一致"
  }
}
```
