...
export const getProcessProdList = (condition) => {
  return (dispatch) => {
    ...
    Req.getProcessProdList(condition)
      .then((res) => {
        if (res.code == 200) {
          dispatch(fetchSuccess({ formData: res.data }));
        }
...
export const saveProcessConfigProduct = (data, id) => {
  return (dispatch) => {
    ...
    Req.saveProcessConfigProduct(data, id)
      .then((res) => {
        hashHistory.push('/process');
      })
      .catch((ex) => {
...
export const getPageFields = (options) => {
  return (dispatch) => {
    ...
    FontConfigReq.getDetail(options)
      .then((res) => {
        if (res.code != 200) return;
